...
func TestRoundRobinEventLoopGroup_Next(t *testing.T) {
	...
	if el0 == nil || el1 == nil || el2 == nil { t.Fatal("Next returned nil") }
	if el3 != el0 { t.Error("round-robin did not wrap around") }
	if g.Count() != 3 { t.Errorf("Count = %d, want 3", g.Count()) }
}
...
func TestRoundRobinEventLoopGroup_SingleWorker(t *testing.T) {
	...
	el0 := g.Next()
	el1 := g.Next()
	if el0 != el1 { t.Error("single-worker group should always return same loop") }
}
