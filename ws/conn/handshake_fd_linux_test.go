...
func TestClientHandshakeFD(t *testing.T) {
	...
	go func() {
		...
		if err != nil { t.Logf("read request: %v", err); return }
		if !strings.EqualFold(req.Header.Get("Upgrade"), "websocket") { t.Error("not ws upgrade"); return }
	...
	if err != nil { t.Fatalf("ClientHandshakeFD: %v", err) }
	<-done
}
